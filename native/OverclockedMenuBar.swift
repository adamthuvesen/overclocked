...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func run(command: URL, arguments: [String]) -> String? {
        ...
        guard process.terminationStatus == 0 else {
            lastError = stderrText?.isEmpty == false ? stderrText : "overclocked exited with status \(process.terminationStatus)"
    ...
    private func rebuildMenu(lines: [String]) {
        ...
        for rawLine in lines.prefix(40).reversed() {
            ...
            item.attributedTitle = NSAttributedString(string: styledLine.text, attributes: styledLine.attributes)
